...
START_TEST(inverse_matrix_test1) {
  matrix_t result;
  ck_assert(s21_inverse_matrix(NULL, &result) == INCORRECT_MATRIX);
}
END_TEST
...
START_TEST(inverse_matrix_test2) {
  ...
  ck_assert(s21_inverse_matrix(&A, &result) == CALCULATION_ERROR);
...
Suite *inverse_matrix_suite(void) {
  ...
  s = suite_create("Test s21_inverse_matrix(matrix_t *A, matrix_t *result)");

  tc_core = tcase_create("inverse matrix");
