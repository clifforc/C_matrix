...
START_TEST(sub_matrix_test2) {
  ...
  s21_create_matrix(rows, columns, &B);
  ck_assert(s21_sub_matrix(&A, &B, &result) == CALCULATION_ERROR);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST
...
START_TEST(sub_matrix_test8) {
  ...
  int rows = 3, columns = 3;
  s21_create_matrix(rows, columns, &A);
  ck_assert(s21_sub_matrix(&A, NULL, &result) == INCORRECT_MATRIX);
  s21_remove_matrix(&A);
}
END_TEST
...
START_TEST(sub_matrix_test9) {
  ...
  int rows = 3, columns = 3;
  s21_create_matrix(rows, columns, &B);
  ck_assert(s21_sub_matrix(NULL, &B, &result) == INCORRECT_MATRIX);
  s21_remove_matrix(&B);
}
END_TEST
...
Suite *sub_matrix_suite(void) {
  ...
  s = suite_create(
      "Test s21_sub_matrix(matrix_t *A, matrix_t *B, matrix_t *result)");

  tc_core = tcase_create("sub matrix");
